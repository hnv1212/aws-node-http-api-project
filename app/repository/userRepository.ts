...
export class UserRepository extends DBOperation{
  ...
  async updateVerificationCode(userId: string, code: number, expiry: Date) {
    const queryString = "UPDATE user SET verification_code=$1, expiry=$2 WHERE user_id=$3 AND verified=FALSE RETURNING *"
  ...
  async updateVerifyUser(userId: string) {
    const queryString = "UDPATE users SET verified=TRUE WHERE user_id=$1 AND verified=FALSE RETURNING *";
    const values = [userId]
    const result = await this.executeQuery(queryString, values)
